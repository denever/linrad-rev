...
// the different frequency drifts we look for.    
    ...
    for(i=0; i<ag.fit_points; i++)
      {    
      drift=-drifti*afc_half_speknum;
      pxy=(void*)(&fftx_xypower[np*fftx_size].x2);
      for(j=0; j<afc_speknum; j++)
        {
        fqp=first_fqp+drift; 
        for(k=0; k<kk; k++)
...
int no_of_points;
float noise1[AG_MAKEP], noise2[AG_MAKEP],signal[AG_MAKEP];
float t1, t2, t3;
float pos;
float *pwra;
float *x;
...
if(k <= 0 || k==no_of_points-1)
  ...
  {
  parabolic_fit(&t1,&pos,sqrt(signal[k-1]),sqrt(signal[k]),sqrt(signal[k+1]));
  mix1_eval_sigpwr[mix1p0+np]=t1*t1;
  mix1_eval_fq[mix1p0+np]=(psref+pos+k)/fftx_points_per_hz;
  }
// Get the noise floor.
// There may be a signal outside the passband.
...
if((float)(abs(new_avgn-afct_avgnum))/(new_avgn+afct_avgnum) <0.15)return;
