...
lir_text(50,6,"L,R => Border left, right");
lir_text(2,7, "  0 => Use raw data without curve fit");
lir_text(2,8, "1-9 => Fit with new number of terms"); 
lir_text(50,7,"  B => Back, set low freq limit");
lir_text(2,9, "  A => Use curve fitted with current no of terms"); 
settextcolor(14);
...
if(lir_inkey == '-')cal_ygain/=2;
if(lir_inkey == 'L')center_region_ia-=0.005;
if(center_region_ia < 0.3)center_region_ia=0.3;
if(lir_inkey == 'R')center_region_ia+=0.005;
if(center_region_ia > 0.49)center_region_ia=0.49;
lir_line(center_region_pix,screen_height*(cal_yzer+YSH),
...
sprintf(s,"Currently %d Hz is excluded from polynomial fit",i);
lir_text(5,6,s);
lir_text(5,7,"as indicated by the green line.");
lir_text(5,9,"L=Change limit");
lir_text(5,10,"C=Continue with current value");
draw_fitline();
...
switch (lir_inkey)
  ...
  case 'L':
  lir_text(5,13,"New limit (Hz):");
  i=lir_get_integer(22,13,6,0,99999);
