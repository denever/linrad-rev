...
dash_sumsq=0;
t1=0;
for(i=0; i<dash_pts; i++)
  {
PRT00"\nDASH WAVEFORM %d %f %f",i,dash_waveform[2*i  ],dash_waveform[2*i+1]);
  
  t2=dash_waveform[2*i  ]*dash_waveform[2*i  ]+
     dash_waveform[2*i+1]*dash_waveform[2*i+1];
  dash_sumsq+=t2;
  if(t1<t2)t1=t2;
  }
t1/=6;
