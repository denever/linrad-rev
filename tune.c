...
void parport_sendwait(char byte)
{    
lir_outb(byte,parport_control);
lir_sleep(PORTWAIT);
}
...
{
char chk;
lir_outb(unit,ui.parport);
lir_sleep(PORTWAIT);
chk=lir_inb(parport_status)&parport_ack;
chk^=parport_ack_sign;
if(bitdat ==0 )
  {
...
// Read and skip data for this time plus 10 milliseconds.
  ...
  i=1+t1*96000*sizeof(int)/rxad.frame;
  i*=rxad.frame;
  i+=rxad.block_bytes;
  while(i>0)
    {
...
// Now read the data that we will process!
  mm=0;
  k=0;
  timf1p_px=(timf1p_pa-timf1_usebytes-2*rxad.block_bytes+
