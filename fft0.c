...
#define SQRTHALF (float)(0.707106781186547524400844362104)

void fft_real_to_hermitian(float *z, int size, int n, COSIN_TABLE *tab)
{
// decimation-in-time, split-radix 
int i, j;
int i1, i2, i3, i4, i5, i6, i7, i8, itab, itab3, inc, inc3;
  ...
  for (j=1; j<n8; j++)
    ...
    do
      ...
      for (i=is; i<size; i+=id)
        ...
        i7 = i6 + n4;
        i8 = i7 + n4;
        t1 = z[i3]*cc1 + z[i7]*ss1;
        t2 = z[i7]*cc1 - z[i3]*ss1;
        t3 = z[i4]*cc3 + z[i8]*ss3;
        t4 = z[i8]*cc3 - z[i4]*ss3;
        t5 = t1 + t3;
        t6 = t2 + t4;
        t3 = t1 - t3;
        t4 = t2 - t4;
...
if(mo == 5)return;  
// Produce an inverted sin power n function over size/2 points
if(mo == 3)
  {
  win[0]=1;
  for (i=1; i <= size/2; i++)
