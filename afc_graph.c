...
t2=0.5*ag.fit_points*(afc_speknum+1)*afc_drift_step/fftx_points_per_hz;
llsq_neq=0;
np=fftx_nf1;
for(i=0; i<ag.fit_points; i++)
  {
...
llsq_derivatives=(void*)(afc_spectrum)+((ag.fit_points*sizeof(float)+15)&0xfffffff0);
llsq_errors=llsq_derivatives+((2*llsq_neq*sizeof(float)+15)&0xfffffff0);  
llsq_npar=2;
k=0;
np=fftx_nf1;
for(i=0; i<ag.fit_points; i++)
  ...
  if(afc_spectrum[i] > t1)
    ...
    if(fabs(wfq-mix1_eval_fq[mix1p0+np]) < t2)
      ...
      llsq_derivatives[llsq_neq+k]=i*afc_spectrum[i];
...
for(i=0; i<ag.fit_points; i++)
  {
  r1=llsq_steps[0]+llsq_steps[1]*i;
  if(r1 < mix1_lowest_fq)r1=mix1_lowest_fq;
  if(r1 > mix1_highest_fq)r1=mix1_highest_fq;
  mix1_fitted_fq[mix1p0+np]=r1;
  t1+=pow((r1-mix1_eval_fq[mix1p0+np]),2.0)*afc_spectrum[i];
  t2+=pow((wfq-mix1_eval_fq[mix1p0+np]),2.0)*afc_spectrum[i];
  r2+=pow((mix1_good_freq[ag_ss]-mix1_eval_fq[mix1p0+np]),2.0)*afc_spectrum[i];
  np=(np+1)&fftxn_mask;
  }
...
t1=wid*baseband_bw_fftxpts;
if( t1 > afcf_search_range/2)
  {
  t1=afcf_search_range/2;
  }
no_of_points=sqrt(t1*t1+25.);
...
if(retcod != 0)
  ...
  while(np != fftx_nf2)
    ...
    if(mix1_eval_sigpwr[mix1p0+np] > 0)
      {
      ag_ston[pb]=log10(mix1_eval_sigpwr[mix1p0+np]/mix1_eval_noise[mix1p0+np]);
      }
    else
      {
      ag_ston[pb]=0;
...
// in order to produce a continous phase
      ...
      mix1_fq_start[mix1p0+fftx_nx]=0.5*(t1+mix1_fq_mid[mix1p0+fftx_nx]);
      kk=t1+0.5;
...
// öö  simple search routine missing!!!!!!!!!!
      ...
      na=(fftx_nx+fftxn_mask)&fftxn_mask;  
...
switch (mouse_active_flag-1)
  ...
  case AG_FQSCALE_EXPAND:
  ag.frange/=2;
  if(ag.frange < AG_FRANGE_MIN)ag.frange = AG_FRANGE_MIN;
  break;

  case AG_FQSCALE_CONTRACT:
  ag.frange*=1.6;
  if(ag.frange > AG_FRANGE_MAX)ag.frange = AG_FRANGE_MAX;
  break;
...
{
ag_ston_y=ag_fpar_y0-ag_floatypix*log10(ag.minston)/AG_STON_RANGE;
if(ag_ston_y < ag_fpar_ytop)
  {
  ag_ston_y = ag_fpar_ytop;
  ag.minston=pow(10.,AG_STON_RANGE*(ag_fpar_y0-ag_ston_y)/ag_floatypix);
  }
}
...
ag.minston=pow(10.,AG_STON_RANGE*k/ag_floatypix);
if(ag.minston < 1.1)
  {
  ag.minston=1.1;
  make_afc_stony();
...
{
ag_lock_y=ag_fpar_y0-2*ag_floatypix*ag.lock_range/ag.frange;
if(ag_lock_y < ag_fpar_ytop)
  {
  ag_lock_y = ag_fpar_ytop;
  ag.lock_range=0.5*(float)(ag_fpar_y0-ag_lock_y)*ag.frange/ag_floatypix;  
  }
}
...
{
ag_srch_y=ag_fpar_y0-ag_floatypix*ag.search_range;
...
if(ag.fit_points <= 1)
  ...
  afc_speknum=ag.fit_points*afcf_max_drift/0.75;
  }
afc_speknum=(afc_speknum&0xfffffffe)+1;  
afc_half_speknum=afc_speknum/2;
afc_drift_step=3./ag.fit_points;
...
ag_size=1.1*(ag.xright-ag.xleft-6*text_width);
...
ag_shift_xpixels=0.9*ag_xpixels;
if(ag_shift_xpixels<32)ag_shift_xpixels=32;
if(ag_shift_xpixels >0.7*ag_xpixels)ag_shift_xpixels=0.7*ag_xpixels;
// ***************************************************************
...
mem1=(2+ag.fit_points)*ag.fit_points+16;
if(mem1<afc_speknum*max_afcf_points)mem1=afc_speknum*max_afcf_points;
if(mem1<max_fftxn)mem1=max_fftxn;
mem2=MAX_MIX1*ag_size*sizeof(float);
ag_old_height=ag_height;
mem3=(text_width+1)*ag_height*sizeof(char);
mem(1,&afc_spectrum,mem1*sizeof(float),0);
if(ui.rx_rf_channels == 2)
  {
  mem(2,&afc_xysum,afc_speknum*max_afcf_points*sizeof(TWOCHAN_POWER),0);
  }  
mem(3,&afc_lowres_spectrum,(1+max_afcf_points)*sizeof(float)/2,0);
mem(4,&ag_mix1freq,mem2,0);
mem(5,&ag_freq,mem2,0);
mem(6,&ag_ston,mem2,0);
mem(7,&afct_window,max_afc_fit*sizeof(float),0);
mem(8,&ag_fitted_fq,mem2,0);
mem(9,&ag_oldpix,AG_CURVNUM*(ag_xpixels+1)*sizeof(short int),0);
mem(10,&ag_background,screen_height*sizeof(char),0);
mem(11,&ag_stonbuf,mem3,0);
mem(12,&ag_srchbuf,mem3,0);
mem(13,&ag_lockbuf,mem3,0);
afc_totmem=memalloc((int*)(&afc_handle),"afc");
if(afc_totmem == 0)
  {
  lirerr(1106);
  return;
...
// Make the default window for the afc graph.
  ag.xright=screen_last_xpixel;
  ag.xleft=ag.xright-0.3*screen_width;
  ag.ybottom=screen_height-1;
  ag.ytop=ag.ybottom-0.3*screen_height;
  ag.mode_control=2;
  ag.avgnum=11;
  ag.fit_points=25;
  ag.delay=17;
  ag.window=1;
  ag.minston=1.5;
  ag.frange=genparm[AFC_LOCK_RANGE]/5;
...
afcf_max_drift=1+genparm[AFC_MAX_DRIFT]*fftx_points_per_hz/60;
afcf_search_range=1+genparm[AFC_LOCK_RANGE]*fftx_points_per_hz;
if(genparm[SECOND_FFT_ENABLE] == 0)
  {
...
max_afcf_points=1.1*afcf_search_range;
if(max_afcf_points < afcf_search_range+20)max_afcf_points=afcf_search_range+20;
