...
hg.clever_bln_limit=2*exp((mouse_x-timf2_hg_xmin)/timf2_hg_xfac)/timf2_hg_xlog;
if(hg.clever_bln_mode == 1)
  {
  hg.clever_bln_factor=hg.clever_bln_limit/timf2_noise_floor;
  if(hg.clever_bln_factor < 0.7)hg.clever_bln_factor=0.7;
  }
hg_control_finish();
}
...
hg.stupid_bln_limit=2*exp((mouse_x-timf2_hg_xmin)/timf2_hg_xfac)/timf2_hg_xlog;
if(hg.stupid_bln_mode == 1)
  {
  hg.stupid_bln_factor=hg.stupid_bln_limit/timf2_noise_floor;
  if(hg.stupid_bln_factor < 0.7) hg.stupid_bln_factor=0.7;
  }
hg_control_finish();
}
...
hg.blanker_ston_fft1=pow(10.,HG_STON_RANGE*k/hg_floatypix);
}
...
hg.blanker_ston_fft2=pow(10.,HG_STON_RANGE*k/hg_floatypix);
}
...
if(leftpressed != BUTTON_RELEASED)return;
leftpressed=BUTTON_IDLE;  
t1=hg_first_fq+(mouse_x-hg_first_xpixel)*hg_hz_per_pixel;
new_mix1_curx[0]=-1;
...
hg_ymax=hg.ytop+1.5*text_height;
...
settextcolor(7);
hg_hz_per_pixel=timf1_sampling_speed/fft2_size;
...
if(hires_handle != NULL)
  {
  memcheck(2,hiresmem,(int*)&hires_handle);
  if(kill_all_flag)return;
  hires_handle=chk_free(hires_handle);
  }
init_memalloc(hiresmem, MAX_HIRES_ARRAYS);
mem(1,&hg_spectrum,ui.rx_rf_channels*screen_width*sizeof(short int),0);
mem(2,&hg_background,screen_height*sizeof(char),0);
mem(3,&hg_stonbuf,text_width*(hg.ybottom-hg.ytop)*sizeof(char),0);
hires_totmem=memalloc((int*)(&hires_handle),"hires");
if(hires_totmem == 0)
  {
  lirerr(1066);
  return;
...
t2=fft2_new_points/timf1_sampling_speed;
t1=hg.spek_avgnum*t2;
if(t1<2)t1=2;
hg_powersum_recalc=hg_size*t2/t1;
...
xt=hg.xleft+2.5*text_width+2;
hgbutt[HG_FFT2_AVGNUM].x1=2+hg.xleft;
hgbutt[HG_FFT2_AVGNUM].x2=2+hg.xleft+4.5*text_width;
hgbutt[HG_FFT2_AVGNUM].y1=2+hg.ytop;
hgbutt[HG_FFT2_AVGNUM].y2=2+hg.ytop+text_height;
hgbutt[HG_SPECTRUM_GAIN].x1=hg.xright-3.5*text_width-2;
...
hgbutt[HG_SPECTRUM_ZERO].x2=xt+3.5*text_width;
hgbutt[HG_SPECTRUM_ZERO].y1=hg_y0-text_height;
hgbutt[HG_SPECTRUM_ZERO].y2=hg_y0;
for(i=0; i<screen_width*ui.rx_rf_channels; i++) hg_spectrum[i]=hg_y0;
if(kill_all_flag) return;
...
for(i=0; i<4; i++)
  {
  timf2_hg_xfac=log(t2/t1)/(timf2_hg_xmax-timf2_hg_xmin+0.5);
  t1=exp(timf2_hg_xfac);
  }
...
t1=100;
timf2_hg_qex=log(timf2_hg_xlog*t1)*timf2_hg_xfac;
...
hg_ston1_y=hg_ston_y0-hg_floatypix*log10(hg.blanker_ston_fft1)/HG_STON_RANGE;
hg_ston2_y=hg_ston_y0-hg_floatypix*log10(hg.blanker_ston_fft2)/HG_STON_RANGE;
hg_ston1_yold=-1;
hg_ston2_yold=-1;
make_blanker_ston1();
make_blanker_ston2();
...
// Make the default window for the high resolution graph.
  hg.xleft=0;
  hg.xright=0.4*screen_width;
  hg.ytop=wg.ybottom+2;
  hg.ybottom=hg.ytop+0.3*screen_height;
