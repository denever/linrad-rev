...
// do not get underflows when computing powers.
  ...
  for(i=ka; i<kb; i++)
    {
    t2=fabs(fft1_filtercorr[mm*i+j  ])+fabs(fft1_filtercorr[mm*i+j+1]);
    if(t2 > t1)t1=t2;
    }
...
// Amplitude into cal_buf2[0] and phase into cal_buf2[1]
  for(i=0; i<fft1_size; i++)
    ...
    if(cal_buf2[mm*i+j  ] == 0)
      ...
      {  
      cal_buf2[mm*i+j+1]=atan2(fft1_filtercorr[mm*i+j+1],
...
for(j=0; j<mm; j+=2)
  ...
  for(i=1; i<fft1_size; i++)
    {
    t1=cal_buf2[mm*i+j]*cos(cal_buf2[mm*i+j+1]);
    t2=cal_buf2[mm*i+j]*sin(cal_buf2[mm*i+j+1]);
    fft1_filtercorr[mm*i+j  ]=t1;
...
k=mm*fft1_size;
t1=0;
for(i=0; i<k; i++)t1+=fabs(fft1_filtercorr[i]);
renorm=fft1_size/t1;
for(j=0; j<mm; j+=2)
  {
  for(i=0; i<fft1_size; i++)
  ...
  for(i=0; i<fft1_size; i++)
    {
    cal_buf[mm*i+j  ]=cal_tmp[2*i  ]/fft1_size;
    cal_buf[mm*i+j+1]=cal_tmp[2*i+1]/fft1_size;
    }
  }  
...
sprintf(s,"File size reduced from %d to %d bytes",
           (1+twice_rxchan)*fft1_size*sizeof(float)+20*sizeof(int), 
           (1+twice_rxchan)*siz*sizeof(float)+20*sizeof(int));
lir_text(5,line+3,s);
lir_text(5,line+5,press_any_key);
await_keyboard();
...
fft2_size=2*cal_interval*(1-SKIP_BEFORE);
...
// Get the total power summed over all channels and store in cal_buf7    
  ...
  for(i=fft1_size/4; i<=3*fft1_size/4; i++)
    {
    cal_buf7[i]=pow(cal_buf[i*mm  ],2.0);
    for(j=1; j<mm; j++)
      {
      cal_buf7[i]+=pow(cal_buf[i*mm+j],2.0);
      }
...
// going further until we looked at a few pulses.
  ...
  if(totbytes > INIT_PULSENUM*cal_interval)
    {
    totbytes=INIT_PULSENUM*cal_interval;
...
// Look for a pulse, at least interval/2 points into our data.
    pulpos=cal_interval/2+fft1_size/4;
    max_pulpos=3*fft1_size/4-cal_interval;
...
find_pulse:;
      ...
      if(show_flag == TRUE)
        {   
        sprintf(s,
        "Accepted %4d Wide %4d Weak %4d S/N %4d Spur %4d  Gain y=%.2f x=%.2f",
                          pulse_error[0],pulse_error[1],pulse_error[2],
                        pulse_error[3],pulse_error[4],cal_ygain,1/cal_xgain);
        lir_text(0,TOPLINE+2,s);
        lir_refresh_screen();
        }
      ia=pulpos-cal_interval/2;
      ib=pulpos+cal_interval/2;
      if(ib < max_pulpos)
        {
        ampmax=0;
        for(i=ia; i<=ib; i++)
          {
          if(cal_buf7[i] > ampmax)
            {
            ampmax=cal_buf7[i];
            pulpos=i;
            }
...
// Also copy the power function to fft1_sumsq with the pulse at location 0
        k=pulpos;
        pulpos+=cal_interval;
        for(i=0; i<fft1_size; i++)
          {
          for(j=0; j<mm; j++)cal_buf2[i*mm+j]=cal_buf[k*mm+j];
          fft1_sumsq[i]=cal_buf7[k];
          if( k<fft1_size/4 || k>3*fft1_size/4 )fft1_sumsq[i]=-1; 
          k=(k+1)&(fft1_size-1);
          }
...
// Now step from the peak position until we reach power/8  
        ...
        ja=0;
        jb=fft1_size-1;    
        while( fft1_sumsq[ja] > ampmax && ja<fft1_size)ja++;
        while( fft1_sumsq[jb] > ampmax && jb>ja)jb--;
...
// Filter the power function to reduce noise. Place in fft1_slowsum.
        width/=2;
        if(width < cal_interval/64)width=cal_interval/64;
        if(width < 4)width=4;
        ja=fft1_size-width;
        jb=width;
        width=2*width+1;
        dsum1=0;
        for(i=0;    i<=jb;     i++)dsum1+=fft1_sumsq[i];
        for(i=ja; i<fft1_size; i++)dsum1+=fft1_sumsq[i];
        for(i=0; i<fft1_size; i++)
          {
          jb=(jb+1)&(fft1_size-1);
          if(jb >= fft1_size)jb-=fft1_size;
          fft1_slowsum[i]=dsum1/width;
          dsum1+=fft1_sumsq[jb]-fft1_sumsq[ja];
          ja=(ja+1)&(fft1_size-1);
          } 
// Locate the surrounding pulses.
        ...
        for(i=ja; i<jb; i++)
          {
          if(fft1_slowsum[i] > t1)
            {
            t1=fft1_slowsum[i];
            ib=i;
            }
          }  
        ja=fft1_size-1.5*cal_interval;
        jb=fft1_size-0.5*cal_interval;
        t1=0;
        ic=ja;
        for(i=ja; i<jb; i++)
          {
...
// skip a range after the previous pulse and before the next one.
        ib=(1.0-SKIP_BEFORE)*ib;
        ic=fft1_size-(fft1_size-ic)*(1.0-SKIP_AFTER);
// When we pick the range ic to ib we may create a discontinuity.
// Remove it by making the first derivative zero.
// Differentiate the pulse response starting at ic:
        mask=fft1_size-1;
        ja=ic;
        jb=ic+1;
        while(ja != ib)
          ...
          for(j=0; j<mm; j++)
            {
            cal_buf2[ja*mm+j] = cal_buf2[ja*mm+j] - cal_buf2[jb*mm+j];
            }
          ja=(ja+1)&mask;
          jb=(jb+1)&mask;
          }  
...
// Integrate the pulse response:
        while(ja != ic)
          ...
          for(j=0; j<mm; j++)
            {
            isum[j]+=cal_buf2[ja*mm+j]-sum[j];
            cal_buf2[ja*mm+j]=isum[j];
            }
          ja=(ja+mask)&mask;
          }  
        ...
        if(cal_domain == 1)
          ...
          for(j=0; j<mm; j+=2)
            ...
            for(i=0; i<screen_width; i++)
              ...
              t2=0.00001*cal_ygain*cal_buf2[mm*k+j];   
              if(t2 <-cal_ymax)t2=-cal_ymax;
              if(t2 >cal_ymax)t2=cal_ymax;
              if(j > 1)t2-=0.32;
              cal_graph[screen_width*j+i]=screen_height*(cal_yzer-t2);
              lir_setpixel(i, cal_graph[screen_width*j+i], 13);  
              lir_setpixel(i, cal_graph[screen_width*(j+1)+i], 0);  
              t2=0.00001*cal_ygain*cal_buf2[mm*k+j+1];   
              if(t2 <-cal_ymax)t2=-cal_ymax;
              if(t2 >cal_ymax)t2=cal_ymax;
              if(j > 1)t2-=0.32;
              cal_graph[screen_width*(j+1)+i]=screen_height*(cal_yzer-t2);
...
// with the corresponding filtered powers in cal_buf6
          ...
          for(i=0; i<siz128+2; i++)
            {
            cal_buf3[mm*i+j  ]=dsum1/n;
            cal_buf3[mm*i+j+1]=dsum2/n;
            cal_buf6[mm*i+j  ]=pow(dsum1/n,2.0)+pow(dsum2/n,2.0);
            }
...
// Also store the noise in cal_buf6
        ...
        for(i=siz128; i<ib; i++)
          ...
          for(j=0; j<mm; j+=2)
            {
            t1=cal_buf6[mm*i+j  ];
            t2=pow(cal_buf2[mm*i+j  ]-cal_buf3[mm*i+j  ],2.0)+
        ...
        for(i=0; i<siz128; i++)
          ...
          for(j=0; j<mm; j+=2)
            {
            cal_buf6[mm*i+j+1]=pow(cal_buf2[mm*i+j  ]-cal_buf3[mm*i+j  ],2.0)+
                               pow(cal_buf2[mm*i+j+1]-cal_buf3[mm*i+j+1],2.0);
            }
          }
        ...
        for(j=0; j<ui.rx_rf_channels; j++)
          {
          t1=-10*log10(dc[2*j]/collect_powermax[j]);
          sprintf(s,"(%.1f)",t1);
        ...
        if(collected_pulses < START_PULSES)
          ...
          for(j=0; j<ui.rx_rf_channels; j++)
            ...
            t1=-10*log10(collect_noiselevel[j]/collect_powerlevel[j]);
...
// raw and filtered data.
        for(i=0; i<fft2_size; i++)
          ...
          for(j=0; j<mm; j+=2)
            {
            t1=pow(cal_buf2[mm*i+j  ],2.0)+
               pow(cal_buf2[mm*i+j+1],2.0);
            if(t1 > 0.001)
              {
              cal_buf2[mm*i+j+1]=atan2(cal_buf2[mm*i+j+1],cal_buf2[mm*i+j  ]);
              }
            else  
              {
              cal_buf2[mm*i+j+1]=0;
...
// Get the first derivative of the phase function
        for(j=0; j<mm; j+=2)
          ...
          for(i=0; i<fft2_size-1; i++)
            ...
            if(cal_buf2[mm*i+j+1] >  PI_L)cal_buf2[mm*i+j+1] -= 2*PI_L;
            if(cal_buf2[mm*i+j+1] < -PI_L)cal_buf2[mm*i+j+1] += 2*PI_L;
            }
          }  
...
spur_remove:;
        ...
        while(ja < fft2_size-siz128  && k==0)
          ...
          for(j=0; j<mm; j+=2)
            ...
            if(fabs(cal_buf2[mm*ja+j+1]-cal_buf2[mm*(ja+1)+j+1])>0.75*PI_L)k=1;
            if(fabs(cal_buf2[mm*ja+j+1]-cal_buf2[mm*(ja-1)+j+1])>0.75*PI_L)k=1;
            }
          }
...
// ja is first bad point  
        ...
        while(jb < fft2_size-siz128  && k==1)
          ...
          k=0;
          jb++;
          for(j=0; j<mm; j+=2)if(dc[j]<cal_buf6[mm*jb+j+1])k=1;
          if(fabs(cal_buf2[mm*jb+j+1]-cal_buf2[mm*(jb+1)+j+1])>0.75*PI_L)k=1;
          if(fabs(cal_buf2[mm*jb+j+1]-cal_buf2[mm*(jb-1)+j+1])>0.75*PI_L)k=1;
          }
...
// If abs of the integral is larger than PI, change the largest point. 
          if(m < 5)
            ...
            if(t1 < -PI_L)
              {
              cal_buf2[mm*ka+j+1]+=2*PI_L;
              goto rem_tupi;
              }
            if(t1 > PI_L)
              {
              cal_buf2[mm*kb+j+1]-=2*PI_L;
              goto rem_tupi;
              }
            }
        ...
        if( (ui.rx_input_mode&IQ_DATA) == 0)
          ...
          for(j=0; j<mm; j+=2)
            ...
            for(i=k; i<fft2_size-k; i++)
              {
              dt3=sqrt(cal_buf4[mm*i+j]*cal_buf4[mm*(i+1)+j]);
              dt1+=dt3*cal_buf4[mm*i+j+1];
              dt2+=dt3;
              }
            dt1/=dt2;  
            for(i=0; i<fft2_size-1; i++) cal_buf4[mm*i+j+1]-=dt1;
            }
          }
        else
          {  
// for complex input we may have a discontinuity at the center
          m=fft2_size/2-k;
          for(j=0; j<mm; j+=2)
            {
            dt1=0;
            ...
            for(i=k; i<m; i++)
              {
              n--;
              dt5=sqrt(cal_buf4[mm*i+j]*cal_buf4[mm*(i+1)+j]);
              dt1+=dt5*cal_buf4[mm*i+j+1];
              dt2+=dt5;
              dt5=sqrt(cal_buf4[mm*n+j]*cal_buf4[mm*(n+1)+j]);
              dt3+=dt5*cal_buf4[mm*n+j+1];
              dt4+=dt5;
              }
            dt1/=dt2;
            dt3/=dt4;
            for(i=0; i<fft2_size/2; i++) cal_buf4[mm*i+j+1]-=dt1;
            for(i=fft2_size/2; i<fft2_size-1; i++) cal_buf4[mm*i+j+1]-=dt3;
            }
          }
...
// Choose it for (phase weighted by power) = 0.
        if( (ui.rx_input_mode&IQ_DATA) == 0)
          ...
          for(j=0; j<mm; j+=2)
            ...
            dt1/=dt2; 
            for(i=0; i<fft2_size; i++) cal_buf4[mm*i+j+1]-=dt1;
            }
          }
        else
// In direct conversion mode there is a discontinuity at the
// center frequency because the A/D converter is not DC coupled
// Treat the two halfs on either side independently
          {
          m=fft2_size/16;
          for(j=0; j<mm; j+=2)
            {
            ...
            for(i=0; i<fft2_size/2; i++)
              {
              k=fft2_size-1-i;
              cal_buf4[mm*i+j+1]-=dt1;
              cal_buf4[mm*k+j+1]-=dt3;
              }
            }
          }
...
// on screen otherwise.
        for(j=0; j<mm; j+=2)
          ...
          for(i=0; i<fft2_size; i++)
            {
            t1=cal_buf4[mm*i+j+1]/(2*PI_L);
            t1-=(int)(t1);
            if(t1 < -0.5)t1+=1;
            if(t1 >  0.5)t1-=1;
            cal_buf4[mm*i+j+1]=t1*2*PI_L;
            }
          }
      if(show_flag == TRUE)
        {   
        ...
        if(cal_domain == 0)
          ...
          for(j=0; j<mm; j+=2)
            ...
            for(i=0; i<screen_width; i++)
              ...
              k*=cal_xgain;
              while(k>fft2_size)k-=fft2_size;
              while(k<0)k+=fft2_size;
              lir_setpixel(i, cal_graph[screen_width*j+i], 0);  
              t2=0.1*cal_ygain*cal_buf4[mm*k+j];   
              if(t2 <-cal_ymax)t2=-cal_ymax;
              if(t2 >cal_ymax)t2=cal_ymax;
              if(j > 1)t2-=0.32;
              cal_graph[screen_width*j+i]=screen_height*(cal_yzer-t2);
              lir_setpixel(i, cal_graph[screen_width*j+i], 13);  
              lir_setpixel(i, cal_graph[screen_width*(j+1)+i], 0);  
              t2=0.03*cal_ygain*cal_buf4[mm*k+j+1];   
              if(t2 <-cal_ymax)t2=-cal_ymax;
              if(t2 >cal_ymax)t2=cal_ymax;
              if(j > 1)t2-=0.32;
              cal_graph[screen_width*(j+1)+i]=screen_height*(0.5-t2);
              lir_setpixel(i, cal_graph[screen_width*(j+1)+i], 10);  
              }
            }
          }
...
if(thread_command_flag[THREAD_CAL_FILTERCORR]==THRFLAG_IDLE)
  ...
  goto idle;
errkbd:;
  thread_command_flag[THREAD_CAL_FILTERCORR]=THRFLAG_IDLE;
idle:;
  while(thread_command_flag[THREAD_CAL_FILTERCORR]==THRFLAG_ACTIVE)
    {
    lir_sleep(3000);
