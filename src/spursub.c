...
// Check that we can locate a spur and move pnt to the peak. 
  ...
  if(fftx_nm < spur_speknum-1)return;
  if(k<9)k=9;
  ia=pnt-k;
  ib=pnt+k;
...
//if(pnt > 1610-SPUR_WIDTH/2 && pnt < 1640-SPUR_WIDTH/2)mailbox[1]=1; else mailbox[1]=0;
//mailbox[1]=1;
spurno=no_of_spurs;
spur_ampl[spurno]=1;
...
ni=(fftx_na+fftxn_mask)&fftxn_mask;
...
ni=(fftx_na-spur_speknum+max_fftxn)&fftxn_mask;
if(sw_onechan)
  {
  if(swmmx_fft2)
    {
...
if(mailbox[1]==1)fprintf( dmp,"\nSUB %d %d:  (%f,%.3f)   (%f,%.3f)    (%f,%.3f)",ni,i,
    ...
    if(swmmx_fft2)
      ...
      errspek[SPUR_WIDTH+1]+=pow(z[2*(SPUR_WIDTH+1)  ],2.0)+
                             pow(z[2*(SPUR_WIDTH+1)+1],2.0)+
                             pow(z[2*(SPUR_WIDTH+1)+2],2.0)+
                             pow(z[2*(SPUR_WIDTH+1)+3],2.0);
      }
    }  
  ni=(ni+fftxn_mask)&fftxn_mask;
  izz--;
