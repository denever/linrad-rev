...
txtest_no_of_segs=(fft1_last_point-fft1_first_point)*fft1_hz_per_point/600;
...
for(i=j; i<=mix1.size/2; i++)
  {
  mix1_fqwin[i]=1;
  }  
j=(mix1.size-mix1.new_points)/2;
k=j;
for(i=0; i<mix1.crossover_points; i++)
  {
...
txtest_spek_no=0;
txtest_spek_p0=0;
while(txtest_first_point-mix1.size/2 < fft1_first_point)
  {
  txtest_first_point+=txtest_pntinc;
  t1+=txtest_pixinc;
  }
txtest_first_xpix=t1+0.5;  
while(txtest_no_of_segs*txtest_pntinc+mix1.size/2+
...
int latest_task, wideflag;
int usercontrol_counter, idle_ensure_counter;
float pwr1, pwr2;
float *z;
float t1;
restart:;
...
usercontrol_counter=0;
idle_ensure_counter=0;
latest_task=0;
wideflag=0;
mix1_flag=0;
...
// transforms so do not wait for a new fft1/fft2 if it is nonzero.
  if(mix1_flag == 0) lir_sem_wait(SEM_FFT1);
...
// because the standard routine we use does not divide for us.
      ...
      txtest_peak_power_decay[i]=
      txtest_peak_power_decay[i]*txtest_decayfac2+
