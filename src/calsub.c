...
char s[80];
clear_screen();
sprintf(s,"Calibration routine for %s. Press F1 for info.",rxmodes[rx_mode]);
...
{
int i, j, k, l, m, n, color,wid,first_pulse;
int p0, ia, ib, ic, ja, jb;
int imode,  lim, overflow_flag;
int rawbytes;
...
rawbytes=ui.rx_ad_speed*rxad.frame;
if(rawbytes > timf1_bytes-3*timf1_blockbytes)
                                      rawbytes=timf1_bytes-3*timf1_blockbytes;
if(rawbytes < 0.25*ui.rx_ad_speed*rxad.frame && fft1_size < 32768)
  {
  lirerr(1241);  
  goto calint_error_exit;
  }
points=rawbytes/rxad.frame;
make_power_of_two(&points);
points >>= 1;
rawbytes=points*rxad.frame;
