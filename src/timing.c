...
// Samples waiting in the baseband 
  r2=(daout_pa+rxda.frame)/(rxda.frame*da_resample_ratio);
...
// Data waiting for output 
  db_wts=(daout_pa-daout_px+daout_bufmask+1)&daout_bufmask;
...
t1=rxda.buffer_bytes-rx_da_maxbytes;
if(t1<0)t1=0;	
t1/=rx_daout_channels*rx_daout_bytes*genparm[DA_OUTPUT_SPEED];
sprintf(s,"%1.3f ",t1);
s[5]=0;
lir_text(19,line,s);
if(measured_da_speed > 0)
  ...
  lir_text(14,screen_last_line-7,s);
  }
