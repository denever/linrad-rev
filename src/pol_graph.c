...
lir_text(5,10,s); 
sprintf(s,"D => Toggle start polarization             %c",sta[istrt]);
lir_text(5,11,s); 
lir_text(5,13,"S => Save to disk and exit");
await_processed_keyboard();
if(kill_all_flag) return;
switch (lir_inkey)
  {
  case 'A':
  lir_text(10,16,"Angle:");
  iang=lir_get_integer(16, 16, 4, 0, 90);
  if(kill_all_flag) return;
  break;
...
pg_default:;
  ...
  dpg.ybottom=screen_height-6*text_height;
  dpg.ytop=dpg.ybottom-PGH;
