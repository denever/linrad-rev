...
if(!freq_from_file)
  {
  sprintf(s,"         %.12f",fg.passband_center+
  FREQ_MHZ_ROUNDCORR);
  i=1;
  while(s[i] != '.' && i< 20)i++;
  k=i-FREQ_MHZ_DIGITS;
...
if(j>wg_waterfall_blank_points)
  ...
  for(ix=0; ix<wg_xpixels; ix++)  
    {
    wg_waterf[local_wg_waterf_ptr+ix]=0;
    lir_setpixel(i,wg_waterf_y,color_scale[0]);
    i++;
    }
  }  
wg_waterf_y--;
...
if(sw_onechan)
  ...
  for(j=hg_first_xpixel; j<hg_last_xpixel; j++)
    ...
    if(ia != ja)
      {
      if(j > hgbutt[HG_SPECTRUM_ZERO].x2 && 
         j < hgbutt[HG_SPECTRUM_GAIN].x1)
        {
  ...
  if(local_workload_counter != workload_counter)
    ...
    for(i=0; i<THREAD_MAX; i++)
      ...
      if(thread_command_flag[i]!=THRFLAG_NOT_ACTIVE)
        {
        if(t1 < thread_workload[i])t1=thread_workload[i];
        if(timinfo_flag)
          { 
          sprintf(s,"%s:%2.2f  ",thread_names[i],thread_workload[i]);
...
fast_screen_end:;
  ...
  if(fabs(sleep_time - recent_time) > 0.3)
    {
    t1=0;
    for(i=0; i<MAX_SC; i++)
      {
    ...
    if(t1 == 0)screen_overload_count=0;
...
// Just in case. Make sure the mouse moves.
  ...
  if(lir_status == LIR_MEMERR)
    {
    wg_error("OUT OF MEMORY",WGERR_MEMORY);
    }
  lir_refresh_screen();
  }
if(thread_command_flag[THREAD_SCREEN]==THRFLAG_IDLE)
