...
for(spurno=0; spurno<no_of_spurs; spurno++)
  ...
  if(swmmx_fft2)
    {
    sptmmx=&spur_table_mmx[spurp0+fftx_na*SPUR_WIDTH*twice_rxchan];
    }
  else
    {
    spt=&spur_table[spurp0+fftx_na*SPUR_WIDTH*twice_rxchan];
    }
  ...
  if(spur_flag[spurno] != 0)
    ...
    if(sw_onechan)
      ...
      if(swmmx_fft2)
        {
        zmmx=&fft2_short_int[2*(fftx_na*fftx_size+spur_location[spurno])];
        ...
        {
        z=&fftx[2*(fftx_na*fftx_size+spur_location[spurno])];
        for(i=0; i<SPUR_WIDTH; i++)
          {
          spt[2*i  ]=z[2*i  ];
          spt[2*i+1]=z[2*i+1];
      ...
      if(swmmx_fft2)
        {
        zmmx=&fft2_short_int[4*(fftx_na*fftx_size+spur_location[spurno])];
        for(i=0; i<SPUR_WIDTH; i++)
          {
          sptmmx[4*i  ]=zmmx[4*i  ];
        ...
        {
        z=&fftx[4*(fftx_na*fftx_size+spur_location[spurno])];
        for(i=0; i<SPUR_WIDTH; i++)
          {
          spt[4*i  ]=z[4*i  ];
          spt[4*i+1]=z[4*i+1];
...
gtpos1:;    
  ...
  zsig[twice_rxchan*fftx_na  ]=r1;
  zsig[twice_rxchan*fftx_na+1]=r2;
  iter=0;
...
// individual spectral lines properly.
  ...
  nx=(fftx_na-spur_speknum+fftxn_mask)&fftxn_mask;
  diffind=0;
  ni=fftx_na;
  while(ni != nx)
    {
...
gtpos3:;
  ...
  if(sw_onechan)
    ...
    if(swmmx_fft2)
      {
      zmmx=&fft2_short_int[4*(fftx_na*fft2_size+spur_location[spurno])];
      for(i=0; i<SPUR_WIDTH; i++)
        {
...
float rot;
float *z, *pwr, *sumsq;
float freq, ampl, phase, phase_slope, phase_curv;
short int *zmmx;
...
spurp0=spurno*spur_block;
np=(fftx_na-spur_speknum+max_fftxn)&fftxn_mask;
nx=(fftx_na+1)&fftxn_mask;
if(swmmx_fft2)
  {
  if(sw_onechan)
...
if(maxrem > spur_speknum)
  {
  maxrem-=spur_speknum;
  nx=(fftx_na-spur_speknum+max_fftxn)&fftxn_mask;
  }
else
  {
  nx=(fftx_na-maxrem+max_fftxn)&fftxn_mask;
  maxrem=0;
  }  
...
while(ni != nx)
  ...
  if(sw_onechan)
    ...
    pxy=(void*)(&fftx_xypower[np*fftx_size+pnt].x2);
    if(swmmx_fft2)
      {
...
if(genparm[SECOND_FFT_ENABLE] == 0)
  ...
  if(sw_onechan)
    ...
    fft1_sumsq_pws=(fft1_sumsq_pws-fft1_size+fft1_sumsq_bufsize)&fft1_sumsq_mask; 
    new_fft1_averages(pnt+1,pnt+SPUR_WIDTH-1);
...
// where n is transform number counted backwards from fftx_na-1
