//background color for signal graphs 

#define LOG_ZERO -0.8
#define LINFAC 0.000025
...
if(powtim_displaymode == 0)
  ...
  if( (ui.rx_input_mode&IQ_DATA) != 0)
    ...
    for(x=0; x<screen_width; x++)
      ...
      ya=ch1_y[x];
      yb=ch2_y[x];
      ya*=adtest_scale*LINFAC;  
      if(ya < -1)ya=-1;
      if(ya >  1)ya= 1; 
      yb*=adtest_scale*LINFAC;  
      if(yb < -1)yb=-1;
      if(yb >  1)yb= 1; 
      ch1_yold[x]=adtest_yzer-ya*adtest_ymax;
      ch2_yold[x]=adtest_yzer-yb*adtest_ymax;
...
ch1_yold=(void*)&fftw_tmp[2*screen_width];
ch2_yold=(void*)&fftw_tmp[3*screen_width];;
...
while(thread_command_flag[THREAD_RX_ADTEST] == THRFLAG_ACTIVE)
  ...
  if(timf1p_px != timf1p_pa)
    ...
    if(points > screen_width)
      {
      points=screen_width;
      timf1p_px=(timf1p_pa-points*rxad.frame+timf1_bytes)&
                                                          timf1_bytemask;
      }                                                          
    p0=(timf1p_px-rxad.block_bytes+timf1_bytes)&timf1_bytemask;
    timf1p_px=timf1p_pa;
    p_increment=rxad.frame;
    p_max=timf1_bytes;
    if(  (ui.rx_input_mode&DWORD_INPUT) == 0)
      {
      p_increment/=sizeof(short int);
      p_max/=sizeof(short int);
      p0/=sizeof(short int);
      }
    else
      {
      p_increment/=sizeof(int);
      p_max/=sizeof(int);
      p0/=sizeof(int);
      }
    p_max--;
